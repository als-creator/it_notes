...
- [18. Чек-лист](#18-чек-лист)
- [19. Практика и задания](#19-практика-и-задания)
...
- [ ] Docker установлен, `systemctl enable --now docker` выполнен
- [ ] Пользователь добавлен в группу `docker`, права перечитываются
- [ ] Сокет `/var/run/docker.sock` не открыт наружу без необходимости
- [ ] Образы сканируются (Trivy), базовые образы и зависимости обновляются

**Dockerfile**

- [ ] Использованы минимальные базовые образы с фиксированными версиями
- [ ] Слои объединены (`RUN ... && ... && rm -rf ...`)
- [ ] Файлы зависимостей копируются до кода (`package.json` → `npm ci` → `COPY . .`)
- [ ] `.dockerignore` исключает `.git`, `node_modules`, `.env`
- [ ] Используется многоступенчатая сборка для компилируемых языков
- [ ] Приложение запускается под `USER` (не root)
- [ ] Секретов нет в `ENV`/`ARG`/образе
- [ ] `CMD`/`ENTRYPOINT` написаны в exec-форме
- [ ] `HEALTHCHECK` задан, логи идут в STDOUT/STDERR
- [ ] `EXPOSE` фиксирует нужные порты

**Запуск и данные**

- [ ] Данные БД в именованных томах (не в записываемом слое)
- [ ] Политика `restart: always` для сервисов, `--rm` для разовых задач
- [ ] Порты проброшены осознанно (`-p`, не всё сразу)
- [ ] Секреты передаются через переменные окружения/environment, а не в коде

**Compose**

- [ ] Файл валиден (`docker compose config`)
- [ ] `depends_on` с `condition: service_healthy` для реальной готовности
- [ ] Порты для внутренних сервисов не публикуются наружу
- [ ] `docker compose down -v` выполняю только понимая, что удалю тома

---

## 19. Практика и задания
